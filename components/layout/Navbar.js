...
export default function Navbar() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-[#173f35]/10 bg-[#f7f7f0]/90 backdrop-blur-xl">
    <nav className="mx-auto flex h-24 max-w-7xl items-center justify-between px-6 lg:px-10" aria-label="Primary navigation">
      ...
      <div className="hidden items-center gap-2 md:flex">{LINKS.map((link) => <Link key={link.href} href={link.href} className="rounded-full px-3 py-2.5 text-base font-semibold text-[#435b54] transition hover:bg-[#e8ece3] hover:text-[#163d34]">{link.label}</Link>)}</div>
      <div className="hidden items-center gap-3 md:flex"><Link href="/login" className="rounded-full px-3 py-2.5 text-base font-semibold text-[#163d34] transition hover:bg-[#e8ece3]">Log in</Link><Link href="/contact" className="rounded-full bg-[#163d34] px-6 py-3.5 text-base font-semibold text-white transition hover:bg-[#285c4f]">Talk to us</Link></div>
