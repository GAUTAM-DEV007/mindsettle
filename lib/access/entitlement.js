...
async function hasFreeVideoClaim(supabase, userId, videoId) {
  const { data } = await supabase
    .from("free_video_claims")
    .select("video_id")
    .eq("user_id", userId)
    .eq("video_id", videoId)
    .maybeSingle();
...
function unavailableAccess() {
  return { allowed: false, requiresLogin: false, requiresUpgrade: false, unavailable: true, freeViewsRemaining: 0 };
}
 ...
 * @param {{ recordView?: boolean, allowUnclaimedFree?: boolean }} [options] - pass recordView: true only
...
export async function resolveVideoAccess(
  ...
  const { recordView = false, allowUnclaimedFree = true } = options;
  ...
  if (user.app_metadata?.must_change_password === true) {
    return { ...unavailableAccess(), requiresPasswordChange: true };
  }
  ...
  if (recordView) {
    // Reservation and limit enforcement happen in one database transaction.
    // A count-then-insert in JavaScript can overspend the cap concurrently.
    const { data, error } = await supabase.rpc("claim_free_video", {
      target_video_id: video.id,
    });
    const claim = data?.[0];
    if (error || !claim) {
      console.error("Could not reserve free playback:", error?.code || "missing_result");
      return unavailableAccess();
    }
    return {
      allowed: claim.allowed === true,
      requiresLogin: false,
      requiresUpgrade: claim.allowed !== true,
      freeViewsRemaining: Math.max(Number(claim.free_views_remaining) || 0, 0),
    };
  }

  const alreadyViewed = await hasFreeVideoClaim(supabase, user.id, video.id);
  const used = await countDistinctFreeViews(supabase, user.id);
  if (used === null) return unavailableAccess();
  ...
  // Hover previews must neither claim a new free video nor reveal its source.
  if (!allowUnclaimedFree) {
    return { allowed: false, requiresLogin: false, requiresUpgrade: false, freeViewsRemaining: Math.max(FREE_VIEW_LIMIT - used, 0) };
  }
  ...
  return {
    ...
    requiresLogin: false,
    requiresUpgrade: false,
    freeViewsRemaining: FREE_VIEW_LIMIT - used,
  };
}
...
export async function getMembershipSummary(
  ...
  const entitled = await hasActiveEntitlement(supabase, 1);
  ...
  return {
    isPaid: false,
    plan: null,
    freeViewsRemaining: used === null ? 0 : Math.max(FREE_VIEW_LIMIT - used, 0),
  };
}
