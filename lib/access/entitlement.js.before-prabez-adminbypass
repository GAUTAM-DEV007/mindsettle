...
async function recordFreeView(supabase, userId, videoId) {
  ...
  if (error) {
    console.error("Could not record free view:", error);
  }
}
 ...
 */
export async function resolveVideoAccess(supabase, user, video, options = {}) {
  const { recordView = false } = options;

  if (!user) {
    return { allowed: false, requiresLogin: true, requiresUpgrade: false, freeViewsRemaining: 0 };
  }
...
export async function getMembershipSummary(supabase, user) {
  if (!user) {
    return { isPaid: false, plan: null, freeViewsRemaining: 0 };
  }
  ...
  if (subscription) {
    return { isPaid: true, plan: subscription.plans ?? null, freeViewsRemaining: null };
  }
