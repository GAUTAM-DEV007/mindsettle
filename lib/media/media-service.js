...
// Rollback must never delete a file still referenced by a saved media row.
// This also protects retries and partial failures in mood assignment cleanup.
export async function deleteUnreferencedMedia({ path }) {
  const supabase = await createClient();
  const references = await Promise.all([
    supabase.from("videos").select("id").eq("video_url", path).limit(1),
    supabase.from("videos").select("id").eq("thumbnail_url", path).limit(1),
  ]);
  if (references.some(({ error, data }) => error || data?.length)) {
    return { skipped: true };
  }
